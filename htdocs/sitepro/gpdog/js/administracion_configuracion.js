...
function obtener_id_usuario() {
    return new Promise((resolve, reject) => {
        const id = sessionStorage.getItem('idSeleccionadousuario');
        if (id) {
            resolve(id);
        } else {
            reject("ID de usuario no encontrado en sessionStorage");
        }
    });
}
    ...
    fetch('../php/cargar_datos_usuario.php', {
        ...
        .then(texto => {
            try {
                ...
                let resumen = document.getElementById("resumen_info");
                document.querySelector("#titulo_username").textContent = data.nombre_usuario;
                resumen.innerHTML = `                <h5>
                    <b>ID usuario</b> ${data.id_usu} <br>
                    <b>Nombre:</b> ${data.nombre} <br>
                    <b>Nombre de usuario:</b> ${data.nombre_usuario} <br>
                    <b>Pedido pendiente:</b> ${data.pedidopendiente}<br>
                    <b>Correo:</b> ${data.correo}<br>
                    <b>Numero de telefono:</b> ${data.num_tel} <br>
                    <b>Direccion:</b> ${data.cp}, ${data.calle}, ${data.municipio}, ${data.estado}, ${data.instrucciones_adicionales}.
                </h5>`;
...
async function enviar_datos_editados() {
    ...
    formData.append('estatus', document.getElementById('estatus_usuario').value);
